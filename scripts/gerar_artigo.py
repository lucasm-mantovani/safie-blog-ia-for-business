...
def gerar_slug(titulo: str) -> str:
    ...
    for origem, destino in [
        ...
        slug = slug.replace(origem, destino)
...
SYSTEM_PROMPT = """Você é um especialista em direito e contabilidade aplicados ao mercado de inteligência artificial e tecnologia disruptiva no Brasil, escrevendo para o blog SAFIE IA for Business (ia.safie.blog.br).

Seu público são founders e gestores de startups de IA, CTOs, diretores de tecnologia, advogados e contadores que atendem empresas de tecnologia, e investidores em deep tech.

Seus artigos seguem o estilo SAFIE: técnico, direto, acessível para empresários e profissionais (não apenas advogados), sem juridiquês excessivo, sem clichês como "no mundo dinâmico", "cada vez mais" ou "em constante evolução".
...
- Parágrafos curtos: máximo 3 linhas. Facilita leitura em mobile"""
...
1. FORMATO BLUF (conclusão primeiro): a PRIMEIRA frase do "resumo_executivo" responde
   ...
   tem mudado...". Certo: "O PL 2.338/2023 cria faixas de risco para sistemas de IA, e
   empresas que operam sistemas de risco elevado terão obrigações de governança."
...
2. KEY TAKEAWAYS: o campo "key_takeaways" traz de 3 a 5 fatos-âncora citáveis do artigo.
   ...
   Certo: "A LGPD (Lei 13.709/2018) se aplica a decisões automatizadas e garante ao
   titular o direito de revisão previsto no art. 20."
...
Apresente a notícia como ponto de partida e aprofunde a análise jurídica e contábil."""
    ...
    return f"""Escreva um artigo completo para o blog SAFIE IA for Business sobre o tema "{tema_nome}".
...
{{
  ...
  "impacto_pratico": "(2 a 3 parágrafos sobre o impacto prático para empresas de IA, founders, CTOs e profissionais, em HTML com tags <p>)",
...
- O FAQ deve ter entre 3 e 5 perguntas reais que founders, CTOs ou advogados fariam ao ChatGPT ou ao Google sobre o tema
...
def montar_artigo_completo(dados_claude: dict, noticia: dict, config_blog: dict) -> dict:
    ...
    url_blog  = config_blog.get("url_completa", "https://ia.safie.blog.br")
    blog_nome = config_blog.get("nome", "SAFIE IA for Business")
